-- Migración: Eliminar price_adjustment con CASCADE
-- Fecha: 2025-09-28
-- Descripción: Elimina correctamente price_adjustment manejando dependencias

-- 1. Primero eliminar la vista que depende de price_adjustment
DROP VIEW IF EXISTS products_with_sizes CASCADE;

-- 2. Ahora eliminar la columna sin problemas de dependencias
ALTER TABLE product_sizes
DROP COLUMN IF EXISTS price_adjustment CASCADE;

-- 3. Recrear la vista sin price_adjustment
...
-- 4. Actualizar función de precio (más simple)
...
-- 6. Verificar que price_adjustment fue eliminado
SELECT
    'price_adjustment eliminado exitosamente' as status,
    'Base de datos limpia y simplificada' as result;

-- 7. Mostrar estructura final limpia
SELECT
    column_name,
    data_type,
    is_nullable,
